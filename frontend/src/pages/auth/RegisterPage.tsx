...
const RegisterPage: React.FC = () => {
  ...
  const handleSubmit = (e: React.FormEvent) => {
    e.preventDefault();
    console.log('Data Register (Dummy):', formData);
    alert('Tombol Sign Up ditekan! Cek console.');
  };
      ...
      <div className="bg-white rounded-2xl shadow-[0_8px_30px_rgb(0,0,0,0.08)] flex flex-col md:flex-row w-full max-w-[900px] border border-gray-200 overflow-hidden">
        ...
        <div className="w-full md:w-1/2 flex flex-col justify-center p-8 md:p-12">
          <div className="text-center mb-8">
            <h2 className="text-[28px] font-bold text-gray-800 mb-1">Welcome 👋</h2>
            <p className="text-sm text-gray-500">
              Sudah punya akun?{' '}
              <Link to="/login" className="text-[#3EA2E8] font-semibold hover:underline">
                Masuk
              </Link>
            </p>
          </div>
          ...
          <form onSubmit={handleSubmit} className="space-y-4">
            <div>
              <label className="block text-sm font-bold text-gray-700 mb-1.5" htmlFor="nama">
                Nama
              </label>
              <input
                type="text"
